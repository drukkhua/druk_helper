...
class UniversalSheetsConverter:
    def __init__(
        self, formats=["csv", "json", "txt"], transliterate=True, enable_caching=True
    ):
    ...
    def _check_format_availability(self):
        ...
        if self.transliterate_names and not SLUGIFY_AVAILABLE:
            print(
                "⚠️ Для универсальной транслитерации рекомендуется: pip install python-slugify"
            )
    ...
    def _get_sheet_hash(self, sheet_data: str) -> str:
        ...
        return hashlib.md5(
            sheet_data.encode("utf-8"), usedforsecurity=False
        ).hexdigest()  # nosec

    def _is_sheet_changed(
        self, sheet_id: str, sheet_name: str, sheet_data: str
    ) -> bool:
    ...
    def load_all_sheets_batch(
        self, sheet_id: str, sheets_info: List[Dict]
        ...
        try:
            ...
            for i, value_range in enumerate(value_ranges):
                ...
                try:
                    ...
                    print(
                        f"✅ [{sheet_name}] {len(df)} строк, {len(df.columns)} колонок"
                    )
            ...
            print(
                f"🎉 Batch загрузка завершена за 1 API запрос! (экономия: {len(ranges) - 1} запросов)"
            )
            return results
        ...
        report = {
            "🎉 ОТЧЕТ О КОНВЕРТАЦИИ": {
                ...
                "⚙️ Настройки конвертации": {
                    ...
                    "Кеширование": (
                        "✅ Включено" if self.stats["caching_enabled"] else "❌ Отключено"
                    ),
                ...
                "📈 Производительность": {
                    ...
                    "API запросов сэкономлено": max(
                        0,
                        self.stats["total_sheets"]
                        - self.stats["batch_requests_used"]
                        - 1,
                    ),
                },
            }
    ...
    def convert_all_sheets(self, url: str) -> bool:
        ...
        try:
            ...
            if processed_count > 0 or cached_count > 0:
                ...
                if cached_count > 0:
                    print(
                        f"⚡ Ускорение благодаря кешу: {cached_count} листов пропущено"
                    )
                return True
            else:
